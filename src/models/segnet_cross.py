"""
SegNet Cross-Stitch model adapted from mtan-reference.
...
from .cost_utils import profile_module, weighted_sum


class SegNetCrossCore(nn.Module):
    """Exact architecture mirror of mtan-reference/im2im_pred/model_segnet_cross.py."""
    ...
    def __init__(self, num_classes: int = 13):
        ...
        self.decoder_block_t = nn.ModuleList(
            [nn.ModuleList([self.conv_layer([filt[0], filt[0], filt[0]], bottle_neck=True)])]
        )
    ...
    def forward(self, x: torch.Tensor) -> Tuple[List[torch.Tensor], torch.Tensor]:
            ...
            for j in range(3):
                if i == 0:
                    ...
                    encoder_cross_stitch = (
                        self.cs_unit_encoder[i - 1][0] * encoder_samp_t[0][i - 1]
        ...
        # decoder with cross-stitch
        for i in range(5):
            for j in range(3):
                if i == 0:
                    decoder_cross_stitch = (
                        self.cs_unit_decoder[i][0] * encoder_samp_t[0][-1]
                    ...
                    decoder_cross_stitch = (
                        self.cs_unit_decoder[i][0] * decoder_conv_t[0][i - 1]
...
class CrossStitchSegNet(BaseMTLModel):
    """Cross-Stitch SegNet wrapper with split points and compression groups."""
    ...
    def __init__(self, config: Dict):
        ...
        self.core_net = SegNetCrossCore(num_classes=num_classes)
    ...
    def partition(self, split_point: int):
        raise NotImplementedError(
            "Direct partition is not implemented for CrossStitchSegNet; use extract_features or hooks."
        )
    ...
    def get_split_flops(self, input_resolution: Tuple[int, int, int]) -> Dict[str, float]:
        ...
        with torch.no_grad():
            encoder_conv = [[None] * 5 for _ in range(3)]
            encoder_samp = [[None] * 5 for _ in range(3)]
            indices = [[None] * 5 for _ in range(3)]
            pool_flops = [[0.0] * 5 for _ in range(3)]

            for i in range(5):
                for task_id in range(3):
                    stage_flops = pool_flops[task_id][i - 1] if i > 0 else 0.0
                    if i == 0:
                        enc_input = x
                    else:
                        enc_input, extra = weighted_sum(
                            [encoder_samp[0][i - 1], encoder_samp[1][i - 1], encoder_samp[2][i - 1]],
                            [self.core_net.cs_unit_encoder[i - 1][0], self.core_net.cs_unit_encoder[i - 1][1], self.core_net.cs_unit_encoder[i - 1][2]],
                        )
                        stage_flops += extra

                    encoder_conv[task_id][i], extra = profile_module(
                        self.core_net.encoder_block_t[task_id][i], enc_input
                    )
                    stage_flops += extra
                    flops[f"task{task_id}_enc_{i}"] = float(stage_flops)

                    (encoder_samp[task_id][i], indices[task_id][i]), pool_flops[task_id][i] = profile_module(
                        self.core_net.down_sampling, encoder_conv[task_id][i]
                    )

            decoder_conv = [[None] * 5 for _ in range(3)]
            for i in range(5):
                for task_id in range(3):
                    stage_flops = pool_flops[task_id][-1] if i == 0 else 0.0
                    if i == 0:
                        dec_input, extra = weighted_sum(
                            [encoder_samp[0][-1], encoder_samp[1][-1], encoder_samp[2][-1]],
                            [self.core_net.cs_unit_decoder[i][0], self.core_net.cs_unit_decoder[i][1], self.core_net.cs_unit_decoder[i][2]],
                        )
                    else:
                        dec_input, extra = weighted_sum(
                            [decoder_conv[0][i - 1], decoder_conv[1][i - 1], decoder_conv[2][i - 1]],
                            [self.core_net.cs_unit_decoder[i][0], self.core_net.cs_unit_decoder[i][1], self.core_net.cs_unit_decoder[i][2]],
                        )
                    stage_flops += extra

                    dec_samp, extra = profile_module(
                        self.core_net.up_sampling, dec_input, indices[task_id][-i - 1]
                    )
                    stage_flops += extra

                    decoder_conv[task_id][i], extra = profile_module(
                        self.core_net.decoder_block_t[task_id][-i - 1], dec_samp
                    )
                    stage_flops += extra
    ...
    def _define_compression_groups(self):
        ...
        for i in range(5):
            compression_groups[f"enc_stage_{i}"] = [f"task{t}_enc_{i}" for t in range(3)]
        for i in range(5):
            compression_groups[f"dec_stage_{i}"] = [f"task{t}_dec_{i}" for t in range(3)]
    ...
    def load_pretrained(self, checkpoint_path: str, strict: bool = True):
        ...
        print(f"[INFO] Loaded pretrained CrossStitchSegNet weights from {checkpoint_path}")
